import logging
import gradio as gr
from app.modules.UI_manager import UI
...
with gr.Blocks() as wl_app:
    ...
    current_files = gr.List(show_label=True, show_row_numbers=True, col_count=None,scale=5,render=False)

    with gr.Column(visible=False) as main_app:
        with gr.Sidebar(label="Select files",open=True,width="45rem"):
                ...
                show_content = gr.Number(label="Characters to display from files",placeholder=250,value=50)
                preview_list = gr.List( 
                    show_label=True, 
                    show_row_numbers=True,
                    col_count=None,
                    headers=["Current documents selected","Sample content"],
                ...
                text_search.submit(
                    ...
                    inputs=[year,type,region,customer,product, text_search, chat_init_uuid, show_content],
                    outputs=[preview_list]
                )
